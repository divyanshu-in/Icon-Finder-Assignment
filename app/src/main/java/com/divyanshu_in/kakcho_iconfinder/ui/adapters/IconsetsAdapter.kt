...
class IconsetsAdapter(
    private val context: Context,
    private val onItemClick: (iconset_id: Int) -> Unit,
) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    private var iconsetsList: ArrayList<ListItemSetsInCategoryData.Iconset> = arrayListOf()
    private var isEndReached = false

    class IconsetsViewHolder(var binding: IconSetItemsBinding) :
        RecyclerView.ViewHolder(binding.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return if (viewType == ICONSETS_VIEW) {
            IconsetsViewHolder(IconSetItemsBinding.inflate(LayoutInflater.from(context),
                parent,
                false))
        } else {
    ...
    override fun getItemCount(): Int =
        if (!isEndReached) iconsetsList.size + 1 else iconsetsList.size

    override fun getItemViewType(position: Int): Int =
        if (position < iconsetsList.size) ICONSETS_VIEW else LOADING_VIEW
